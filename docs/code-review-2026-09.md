...
- [ ] `Rendering/MathRenderer.swift` `prepare` —— 渲染完成后的 `Task.checkCancellation()` 排在 `cache.setObject` **之前**，而 `evaluateJavaScript` 的 continuation 没有取消钩子、必然跑完。`scheduleParse` 每次击键都 cancel `mathPreparationTask`，于是连续输入时每次都付出完整 WebKit 往返再丢弃，公式永远渲染不出来。修：产物先入缓存，再判取消。
- [ ] `Document/RenderCoordinator.swift` `scheduleParse` / `scheduleMathPreparation` —— `needsMathRefresh` 在编辑（`scheduleParse`）、图片刷新（`applyImageRefreshIfPossible`）、模式切换（`applyPresentationModeIfPossible`）三处被清，而 `prepare` 只在**新建**产物时返回 true。「产物已进缓存但未写入 storage」这个状态在标志设计里不可表达，一旦错过刷新窗口，该公式整个会话都停在裸 `$$…$$`（`applyDirty` 只覆盖脏行，`reconcileVisibility` 也只在 `cached != entry.state` 时写）。修：信号改为表达「产物可用」而非「本次新建」，刷新标志只在刷新真正执行后清除。参考图片路径在 `apply` 里消费 `needsImageRefresh` 的做法。
