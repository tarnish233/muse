...
**要做**：先写一个**会失败**的测试把问题固定下来（红），再由 M2-8 修复转绿。

- 测试名：`blockVisualsFollowAppearance`
- 做法：在 `.aqua` 与 `.darkAqua` 两种 `NSAppearance.current` 下分别经真实 fragment 绘制引用块到位图，采样引用背景区域的像素
- 断言：两次采样的颜色**不同**
- 若当前实现下该测试失败，用 `withKnownIssue` 标注并注明「待 M2-8 修复」，不要用 `#expect` 反向断言把 bug 写成规格

**验收**：测试存在；M2-8 完成后移除 `withKnownIssue` 且通过。

**完成记录（2026-08-27）**：已完成。`blockVisualsFollowAppearance` 已移除
`withKnownIssue`，通过真实 fragment 分别在浅色/深色外观下采样引用背景；
`paletteUpdatesOnAppearanceChange` 也通过。对应真机浅深色截图已记录在 M2 报告 §6.4。

### 9.4 任务 M0-3：人工验收清单（**需要人执行，Codex 无法替代，⏳ 待人工**）
...
第 12/13 项若失败，即确认 M2-8 的隐患在真机成立，请在《M2 评价报告》§6.4 回填「真机确认：失败」。

**状态（2026-08-27）**：M0-3 仍待人工执行。上表“实测结果”列按要求保留为空，Codex
未代替人眼/人手判断中文 IME、VoiceOver 与零宽留白观感。
